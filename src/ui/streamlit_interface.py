...
from utils.monitoring import ConversationLogManager
...
def main():
    ...
    MANAGER_STATE_KEY = "conversation_log_manager"
    ...
    if MANAGER_STATE_KEY not in st.session_state:
        conversation_id = datetime.now().strftime("%Y%m%d%H%M%S")
        st.session_state[MANAGER_STATE_KEY] = ConversationLogManager(conversation_id)

    log_manager: ConversationLogManager = st.session_state[MANAGER_STATE_KEY]
        ...
        try:
            with st.chat_message("assistant"):
                with st.spinner("Tutor is formulating an answer..."):
                    ...
                    log_manager.add_turn(str(prompt), str(response))
                    log_manager.export_conversation()
