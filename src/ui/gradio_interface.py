...
def generate_response(message, history):
    ...
    messages_history = [{"role": "user" if i % 2 == 0 else "assistant", "content": msg} 
                        for i, (msg, _) in enumerate(history)] + [{"role": "user", "content": message}]
...
with gr.Blocks() as demo:
    ...
    upload_status = gr.Textbox(
        label="Status do carregamento",
        interactive=False
    )
    
    chat = gr.ChatInterface(
        fn=generate_response,
        examples=["Qual é o tema principal deste documento?", 
