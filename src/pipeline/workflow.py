...
class RunPipeline:
    ...
    def get_context(self):
        latest_messages = st.session_state.get("messages", [])[-self.SHORT_TERM_MEMORY:]
        context = ""
    ...
    def start_rag(self, pdf_path, temperature=0.7, top_k=0.0, top_p=0.0):
        ...
        key = api_key.get_key()
        try:
            self.rag_instance = RetrievalAugmentedGeneration(key, pdf_path, temperature, top_k, top_p)
